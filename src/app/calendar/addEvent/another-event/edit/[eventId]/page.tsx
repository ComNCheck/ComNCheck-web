...
export default function AnotherEventEdit() {
  ...
  useEffect(() => {
    const loadEventData = async () => {
      try {
        const result = await getEvent("another", eventId);
        if (result.success && result.data) {
          ...
          const mockData: EventData = {
            ...
            endDate: "2024-05-22",
            location: "공과대학 대강당",
            announcement: "공과대학 학술제 공지사항입니다.",
            googleFormLink: "https://forms.google.com/academic-festival",
            cardNewsLink: "https://example.com/academic-cardnews",
          };
  ...
  const handleUpdate = async (data: any) => {
    try {
      // TODO: 실제 API 호출로 대체
      // const response = await fetch(`/api/another-events/${eventId}`, {
      //   method: 'PUT',
